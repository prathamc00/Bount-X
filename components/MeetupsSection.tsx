...
const MeetupCard: React.FC<{ event: MeetupEvent, index: number }> = ({ event, index }) => {
  ...
  return (
    ...
    >
      <div
        ref={innerCardRef}
        className="bordered-card rounded-xl p-6 flex flex-col h-full"
        style={{ transform: `rotateX(${rotate.y}deg) rotateY(${rotate.x}deg) scale(1.02)`, transition: 'transform 0.2s' }}
      >
        <div className="flex-grow">
          <div className="flex justify-between items-start mb-3">
            <h3 className="text-xl font-bold text-white">{event.title}</h3>
            <span className="text-xs font-mono bg-fuchsia-400/10 text-fuchsia-400 px-2 py-1 rounded whitespace-nowrap">{event.topic}</span>
          </div>
        ...
        <a href={event.rsvpUrl} target="_blank" rel="noopener noreferrer" className="mt-auto block w-full text-center px-4 py-2 bg-slate-700 text-white font-semibold rounded-md hover:bg-fuchsia-600 hover:text-white transition-colors duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-fuchsia-500 focus-visible:ring-offset-slate-800">
          RSVP / Details
        </a>
      </div>
    </div>
    ...
    <section id="meetups" className="py-20 sm:py-28">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div ref={ref} className={`text-center mb-12 ${isVisible ? 'fade-in visible' : 'fade-in'}`}>
          <h2 className="text-4xl md:text-5xl font-bold font-mono text-white tracking-tight">Offline Meetups</h2>
